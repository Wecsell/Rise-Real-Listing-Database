...
Поле хранит пару как "долгота, широта" (для Бали начинается с 115), а полевой
бот всё это время писал "широта, долгота" — так их отдаёт Telegram.

Карта на странице Interfaces читает это поле как есть, в порядке
"долгота, широта", и точки встают верно — проверено в живом интерфейсе
06.08.2026. Второе поле под карту не нужно.

Если карта пуста — она ещё геокодирует (после сброса её кэша полный проход
занимает минуты). Не принимать это за ошибку порядка: 06.08.2026 такая ошибка
диагноза стоила семи переписываний живых координат.
